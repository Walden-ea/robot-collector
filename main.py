...
def main():
    ...
    for i in range (3):
        cont_coords[i] = ww.generate_coords()

    for i in range(3):
        cont.append(ct.Container(i, cont_coords[i][0],cont_coords[i][1]))

    container_group = pg.sprite.Group()
    for c in cont:
        container_group.add(c)

    robot = rb.Robot(5, 20.0,WINDOW_WIDTH, WINDOW_HIGHT, container_group, sensor_length=40, x=WINDOW_WIDTH / 2, y=WINDOW_HIGHT / 2)
    ...
    while True:
        ...
        for s in robot.sensors:
            c_sprite = pg.sprite.spritecollide(s, container_group, False, pg.sprite.collide_mask)
            sprite = pg.sprite.spritecollide(s, garb_group, True, pg.sprite.collide_mask)
            if pg.sprite.spritecollide(s, walls, False, pg.sprite.collide_mask):
                ...
                robot.capacity-= sprite[0].volume
                robot.c_tag = sprite[0].tag
                print("COLLIDES")
